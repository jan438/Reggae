...
def drawLegendLines(d, i):
    ...
    d.add(String(leftmargin + float(legendsdata[i][1]) - dx - 20.0 + 0.5 * (maxnamewidth - namewidth), bottommargin + float(legendsdata[i][2]) - 83, legendsdata[i][0], font = reggaefont, fillColor = HexColor(background2), fontSize = 12))
    return
...
for i in range(len(legendsdata)):
    drawLegendLines(d, i)
renderPDF.drawToFile(d, 'PDF/ReggaeBorderless17.pdf')
