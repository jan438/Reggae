...
from reportlab.lib.pagesizes import LETTER, A3, landscape, portrait
...
legendsdata = []
s = 100
dx = 0.5 * s                         #                            width hexagon  200
dy = math.sqrt(s**2 - (0.5 * s)**2)  # sqrt(10000 - 2500) = 86.60 height hexagon 173.21 ratio 1.154701077
strokew = 10
...
def drawLegendTriangle(d, i):
    ...
    d.add(Image(path = img, width = 200.05, height = 173.25, x = leftmargin + float(legendsdata[i][1]) - 6.5 - dx - 0.5 * dy, y = bottommargin + float(legendsdata[i][2]) - 86.5, mask = None))
...
def drawLegendLines(d, i):
    ...
    d.add(h)
    namewidth = pdfmetrics.stringWidth(legendsdata[i][0], reggaefont, 17)
    print(legendsdata[i][0], namewidth)
    d.add(String(leftmargin + float(legendsdata[i][1]) - dx + 10.0 + 0.5 * (maxnamewidth - namewidth), bottommargin + float(legendsdata[i][2]) - 115, legendsdata[i][0], font = reggaefont, fillColor = HexColor(background2), fontSize = 17))
    return
...
d.add(transform_svg("rasta.svg", 130.0, 760.0, 0.55, 0.55))
d.add(transform_svg("cannabis.svg", 90.0, 612, 0.035, 0.035))
d.add(transform_svg("cannabis.svg", 120.0, 612, 0.035, 0.035))
d.add(transform_svg("cannabis.svg", 150.0, 612, 0.035, 0.035))
for i in range(len(legendsdata)):
    drawLegendLines(d, i)
